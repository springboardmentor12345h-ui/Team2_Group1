...
exports.getSuggestions = async (req, res) => {
  try {
    ...
    if (!search) return res.status(200).json({ status: 'success', data: [] });
    ...
    events.forEach(ev => {
      if (ev.title.toLowerCase().includes(search.toLowerCase())) suggestions.add(ev.title);
      if (ev.category.toLowerCase().includes(search.toLowerCase())) suggestions.add(ev.category);
    });

    res.status(200).json({
      status: 'success',
      data: Array.from(suggestions).slice(0, 8) // Top 6 unique suggestions
    });
  } catch (error) {
