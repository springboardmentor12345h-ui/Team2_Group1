...
useEffect(() => {
  const timeoutId = setTimeout(async () => {
    fetchEvents(); // Aapka purana function
    
    // Naya: Suggestions Fetch karne ke liye
    if (query.trim().length > 1) {
      try {
        const { data } = await axios.get(`/api/v1/events/suggestions?search=${query}`);
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              <Input
                ...
                placeholder="Search events by title, description or category..."
                className="pl-10 h-12 w-full border-2  focus:border-purple-500 rounded-xl"
              />
              <MagnifyingGlassIcon className="w-5 h-5 text-secondary-400 absolute left-3 top-3.5" />

              {showSuggestions && suggestions?.length > 0 && (
                <div className="w-full top-full left-0 right-0 top-full mt-1 bg-white border-2 border-purple-500 rounded-xl shadow-[0_20px_50px_rbga(0,0,0,0.3)] z-[9999] overflow-hidden">
                  {suggestions.map((suggestion, index) => (
                    <div
                      key={index}
                      className="px-4 py-3 hover:bg-purple-50 cursor-pointertext-gray-700 border-b border-purple-100last:border-0 font-medium"
                      onMouseDown={(e) => {
                        e.preventDefault();
                        console.log("Selecting:",suggestion);
                        setQuery(suggestion);
                        setShowSuggestions(false);  
                        fetchEvents (suggestion);
                      }}
                    >
