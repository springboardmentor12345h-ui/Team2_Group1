...
const LaraChatbot = ({ user }) => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: `Hi ${user?.name?.split(" ")[0]}! I'm Lara, your Campus Pulse assistant. How can I help you today?`,
      sender: "lara",
    },
  ]);
  ...
  const predefinedQA = [
    {
      q: "What is Campus Pulse?",
      a: "Campus Pulse is a centralized platform where colleges host inter-college events like Sports, Hackathons, Cultural Fests, and Workshops. It's your one-stop shop for campus engagement!",
    },
    {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] font-sans">
      {/* Chat Window */}
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-80 sm:w-96 h-[500px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-primary-100 transition-all duration-300 transform origin-bottom-right">
          {/* Header */}
          <div className="bg-gradient-to-r from-primary-600 to-indigo-600 p-4 flex justify-between items-center">
            <div className="flex items-center space-x-3 text-white">
              ...
              <div>
                ...
                <p className="text-[10px] text-primary-100 uppercase tracking-widest font-bold">
                  Virtual Assistant
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-primary-50/30">
                ...
                <div
                  className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${
                    msg.sender === "user"
                      ...
                      : "bg-white text-gray-800 border border-primary-100 rounded-tl-none"
                  }`}
                >
                  {msg.text}
                </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white border border-primary-100 p-3 rounded-2xl rounded-tl-none flex space-x-1">
                  ...
                  <div className="w-2 h-2 bg-primary-500 rounded-full animate-bounce [animation-delay:-.5s]"></div>
                </div>
              </div>
            )}
          ...
          {/* Quick Questions */}
          <div className="p-3 border-t border-primary-100 bg-white">
            ...
            <div className="flex flex-wrap gap-2">
              {predefinedQA.map((qa, index) => (
                <button
                  ...
                  className="text-xs bg-primary-50 text-primary-700 px-3 py-1.5 rounded-full hover:bg-primary-100 border border-primary-200 transition-colors text-left font-medium"
                >
                  {qa.q}
                </button>
              ))}
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all duration-500 hover:scale-110 active:scale-95 ${
          isOpen
            ? "bg-white text-primary-600 border border-primary-100 rotate-90"
